/*****************************************************************************
 ...
 @Details  : STM32F722XX Timer driver               
 @Author   : Liang Tang
 @Hardware : STM32F722
 ...
 --------------------------------------------------------------------------
   ...
   1.1  Liang     2018-09-20   Remove some key parts and students are required
                               to fill.
******************************************************************************/
#include <Common.h>
#include "Hal.h"
...
int TimerInit( PTIMER_HANDLE pHandle, uint8_t nTimer, uint32_t nFreq )
{
  ...
  /********************************************************
  * To do: ...
  * Based on the Timer module input clock frequency and
  * the required frequency, decide the Timer prescaler (PSC)
  * and auto-reload register (ARR) registers.
  ********************************************************/
   if ((RCC->DCKCFGR1 & RCC_DCKCFGR1_TIMPRE) == 1)
  {
      clock =  4 * clock;
  }
  ...
  while(count >= 0xFFFF){
    ...
    count = clock / nFreq; //reload value for the ARR

    prescale++; //increase prescale for every time the count exceed the reload value

    count = count / (prescale + 1); //The counter clock frequency (CK_CNT) is equal to fCK_PSC / (PSC[15:0] + 1)
    }

    if (prescale > 0)
    {

       timer->PSC  = prescale; //setting the prescale value in the PSC register
       
    }

     timer->ARR =  count; //setting the reload value in the ARR

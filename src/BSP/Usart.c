  ...
  )
{
  ...
  ********************************************************/
  ...
  uart->CR1 |= USART_CR1_TXEIE; //TX empty interrupt is enabled 
  
}

/*direct memory access, is an interfacing approach that transfers data directly to/from memory. 
With an input device, the hardware will request a DMA transfer when the input device has new data. */
static void  UartDmaTx( void *pHandle, void const *pData, int nSize ) //
{
  UART_HANDLE const   *handle = (UART_HANDLE const *)pHandle;
  USART_TypeDef       *uart   = (USART_TypeDef *)handle->pUART;
...
static void uart_Handler( PUART_HANDLE pHandle, USART_TypeDef *pUart ) //ISR handler to ensure that transmission continues after the 1st transmission, if there is more data to send.
{
  int status;
  int tmp;
  ...
  /* Check for Tx IRQ */
  if( 0 != (status & USART_ISR_TC) ) //check for ISR status and transmit status, see if transmission is done.
  {
    UartIsrTx( pHandle );
  }
